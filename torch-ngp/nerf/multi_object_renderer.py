...
def selective_mask_t(image_src, mask):
    mask = mask[:, ]
    mask = torch.sign(torch.sum(mask, dim=1)).to(dtype=image_src.dtype).unsqueeze(-1)
...
class MultiObjectNeRFRenderer(nn.Module):
    def __init__(self,
                 bound=1,
                 cuda_ray=False,
                 density_scale=1,
    ...
    def run(self, rays_o, rays_d, label="all", num_steps=128, upsample_steps=128, bg_color=None,
            perturb=False, **kwargs):
        ...
        # query SDF and RGB
        density_outputs = self.density(xyzs.reshape(-1, 3), label=label)
        ...
        if upsample_steps > 0:
            ...
            new_density_outputs = self.density(new_xyzs.reshape(-1, 3), label=label)
        ...
        rgbs = self.color(xyzs.reshape(-1, 3), dirs.reshape(-1, 3), mask=mask.reshape(-1), label=label,
                          **density_outputs)
    ...
    def train_render(self, rays_o, rays_d, inds, loss=None, gt_rgb=None, staged=False, max_ray_batch=4096, **kwargs):
        ...
        for idx in range(len(detected_objects) + 1):
            if results:
                ...
                #         while head < N:
                #             tail = min(head + max_ray_batch, N)
                #             results_ = _run(rays_o[b:b + 1, head:tail], rays_d[b:b + 1, head:tail], object, **kwargs)
                #             depth[b:b + 1, head:tail] = results_['depth']
                #             image[b:b + 1, head:tail] = results_['image']
                #             head += max_ray_batch
                #
