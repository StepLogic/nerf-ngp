...
def extract_features(image):
    ...
    # yolo.eval()
    # yolo.requires_grad_(False)
    return yolo(image, verbose=False)
...
def get_masked_rays(poses, intrinsics, H, W, N=-1, error_map=None, patch_size=1):
    ''' get rays
    Args:
        poses: [B, 4, 4], cam2world
        intrinsics: [4]
...
class Trainer(object):
    ...
    def train_step(self, data):
        ...
        assert 'images' in data and "inds" in data
        #     B, N = rays_o.shape[:2]
        #     H, W = data['H'], data['W']
        #
        #     # currently fix white bg, MUST force all rays!
        #     outputs = self.model.train_render(rays_o, rays_d, staged=False, bg_color=None, perturb=True, force_all_rays=True,
        #                                 **vars(self.opt))
        ...
        assert "gt_rgb" in outputs
        pred_rgb = outputs['image']
        gt_rgb = outputs["gt_rgb"]

        # MSE loss
        loss = outputs["loss"].mean(-1)  # [B, N, 3] --> [B, N]
